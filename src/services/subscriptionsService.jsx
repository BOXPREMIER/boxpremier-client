...
export async function fetchActivePlans() {
  ...
  const { data } = await JsonServerAPI.get('/subscriptions/plans', { params: { active: true } })
  return data
}
...
export async function createSubscription({ planId, boxType, wineType, payMethod = 'multisafepay' }) {
  ...
  const { data } = await JsonServerAPI.post('/subscriptions', payload)
  return data
}

export async function getMyActiveSubscription() {
  const { data } = await JsonServerAPI.get('/subscriptions/me/active')
  return data
}

export async function updateSubscription(subId, patch) {
  const { data } = await JsonServerAPI.patch(`/subscriptions/${subId}`, patch)
  return data
}

export async function cancelSubscription(subId) {
  const { data } = await JsonServerAPI.post(`/subscriptions/${subId}/cancel`)
  return data
}
